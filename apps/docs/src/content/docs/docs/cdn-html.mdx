...
<body sz="{ p: 8, bg: 'slate-950', color: 'slate-200', minH: 'screen' }">
    <h1 sz="{ text: '4xl', weight: 'bold', color: 'blue-500', mb: 4 }">
        Hello csszyx
    </h1>
...
csszyx has written them, so the Tailwind script comes first in the
`<head>`.
...
This is automatically injected by `npx @csszyx/cli migrate public/` when
