...
That `@source` is your package root, relative to this CSS file. `source(none)` turns
...
<Aside type="caution" title="Globs and CSS comments do not mix">
A CSS block comment ends at the **first** `*` followed by `/` — and every recursive
glob contains one. Writing `@source "src/**/*.md"` inside a `/* … */` comment closes
the comment early, and the rest of your stylesheet parses as declarations. Keep globs
in the directive, not in the note about it.
</Aside>
