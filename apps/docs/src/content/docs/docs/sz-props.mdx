...
Only truthy objects contribute classes. Prefer `undefined` over an empty
object for "no styling": `sz={cond ? { pl: 4 } : undefined}` — an empty
object branch compiles to an empty class string (`class=""` in the output),
while `undefined` keeps the output clean.
:::
