...
:::caution[Mangling is unsafe in a hybrid build — keep `mangle: false`]
Mangling assumes csszyx owns the entire class namespace. That breaks when a
**separate Tailwind plugin owns the utility CSS** and/or **hand-written CSS uses
literal class names**, in two ways:
...
From the version that adds it, a production mangle build **warns** when it detects
either hazard (the colliding tokens / orphaned classes), but does not auto-fix
them. Until your class namespaces are reconciled, the correct setting for a hybrid
app is **`production: { mangle: false }`**.
:::
