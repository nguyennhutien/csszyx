...
| `parser`         | `'oxc'`                | Source parser for JSX/TSX sz transforms. Use `'babel'` only as a compatibility escape hatch if an app hits an oxc parser edge case. Use `'rust'` to opt into the native Rust engine; it requires the matching optional `@csszyx/core-*` platform package. |
