...
Mangling is a **production-only** step, applied at **build time**. It assumes
csszyx owns the entire class namespace, which breaks when a **separate Tailwind
plugin owns the utility CSS** and/or **hand-written CSS uses literal class names**,
in two ways:
...
3. **Exclude — escape hatch for code you can't change.** Only when a name lives in
   a **third-party stylesheet** you cannot rename, reserve it so no token equals
   it:
