...
verify the CSS a build emits. Classes that also appear as static literals in
`class`/`className` (including `clsx(...)` expressions) automatically retain their
original names, so raw and `sz` consumers keep matching the same CSS rule. To check
a single `sz` object without a build, run
...
3. **Exclude — escape hatch for code you can't change.** Only when a name lives in
   a **third-party stylesheet** you cannot rename, or is assembled dynamically
   outside a statically visible class expression, reserve it so no token equals it:
