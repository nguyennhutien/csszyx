...
Keep build output outside the roots the prescan walks. It reads any source
under the root as authored code, so a kept `dist-previous/` of an earlier
mangled build hands back `y4` and `n4` as class names, and the next build
declines to mangle anything that might collide with them. The map shrinks,
the emitted CSS changes and the build stays green. A `dist` entry in
`.gitignore` does not cover `dist-mangle-on`; name the directories, or keep
