...
`.csszyx/mangle-runtime.mjs` on webpack — attached to every HTML entry ahead of
the app and to every source module that imports the runtime, so a pre-compiled
