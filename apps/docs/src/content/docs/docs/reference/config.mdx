...
**Nothing outside your build can bind to a class name.** A token is allocated
from the set of classes the build found, so it moves whenever that set changes.
A browser extension, a host page your app is embedded in, another team's E2E
selectors, a scraper: none of them can hold onto `bg-red-500` across two
deploys. Mangling is how a build enforces "do not depend on my class names".
It is not confidentiality — the stylesheet ships, so `.z{padding:1rem}` reads
plainly, and the census in the page maps every token back to the name it came
from.
