...
unaffected. Narrow this only against a deployment shape you know: if the page never
receives the delivery you picked, runtime helpers silently fall back to original
class names while the CSS ships mangled — the styles simply do not apply. When in
doubt, leave it at `'both'`.
...
interface BuildConfig {
  ...
  warn?: boolean; // Emit build advisory diagnostics (default true)
...
| `warn`           | `true`                 | Emit build advisory diagnostics: the sz runtime-fallback matrix (why an `sz` expression could not be compiled, and whether `szv()` or `dynamic()` is the way out), unresolvable spreads, `szs`/`szRecover` shape notices, and the unknown-key/dead-value scans. Wording is identical on every parser engine, so flipping `build.parser` never changes the build log. Set `false` to run the transform as a single pass with none of that machinery. Hard failures where output was withheld (parser errors, the AST-budget bail) are not advisory and are reported regardless. |
