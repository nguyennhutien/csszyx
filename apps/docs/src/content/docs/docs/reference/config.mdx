...
| `parser`         | `'oxc'`                | Source parser for JSX/TSX sz transforms. Use `'babel'` only as a compatibility escape hatch if an app hits an oxc parser edge case. `'rust'` is an experimental scaffold and currently throws until the Rust core lands. |
