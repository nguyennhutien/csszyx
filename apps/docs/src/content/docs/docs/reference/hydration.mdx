...
When `production.mangleVars` is enabled, the checksum payload includes both
class and CSS-variable entries. One original dynamic variable can map to more
than one mangled name, so variable entries are namespaced with the mangled name
in the checksum key.
...
Production builds with debug helper injection expose `window.__csszyx` for
inspection. `mangleMap` stays class-only, while `varMangleMap` contains CSS
variable mappings when `production.mangleVars` is enabled.
