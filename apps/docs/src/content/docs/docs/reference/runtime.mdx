...
- **`@csszyx/runtime/core`** — the concat family (`szr`, `_sz`, `_sz2`, `_sz3`,
  `__szvPick`), string-first, no compiler (~0.6&nbsp;KB gz). When every `szr(...)`
...
object merge keeps one class where concatenation keeps both. The cross-module
half runs in production builds only and resolves RELATIVE import specifiers;
dev keeps the unoptimized behavior so nothing can go stale under HMR.
