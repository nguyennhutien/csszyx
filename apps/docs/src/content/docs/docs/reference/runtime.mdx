...
**Next.js with Turbopack is the exception.** That lane compiles one file at a
time through a loader and never injects the generated registration, so custom
tokens do not join their groups there — `szcn` keeps both classes and the
stylesheet order decides. Call
[`registerSzcnGroups`](#registerszcngroupsgroups) once at app startup, or use
Next's Webpack mode for full parity.
:::
