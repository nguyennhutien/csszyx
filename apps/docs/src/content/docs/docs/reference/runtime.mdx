...
description: szr (sz-resolve), _szMerge, szv() composition and variant helpers, plus splitBox, classify/has/pick/omit, stripSzProps, and verifyMangleChecksumAsync.
---
...
<CardGrid>
  <Card title="Runtime helpers" icon="puzzle">
    `szr` · `szcn` · `splitBox` + class toolkit · SSR hydration validator
  </Card>
...
Narrows a compiled [`szs`](/docs/compound-components/#the-szs-prop--style-internal-parts-in-one-prop)
slot to its class string. Authored as an sz value, every slot is compiled to a
string before the component runs — `szsClass` does the type narrowing once and
fail-safe: an uncompiled slot (misconfigured build) returns `undefined` instead
of coercing into `class="[object Object]"`.

```ts
function szsClass(slot: unknown): string | undefined
```
...
function Card({ szs }: { szs?: Szs<'title' | 'body'> }) {
  ...
  return <h3 className={szcn('font-medium text-base', szsClass(szs?.title))} />;
}
```
