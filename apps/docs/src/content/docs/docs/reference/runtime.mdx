...
modules that call `szcn`. The one difference is refresh — the file is generated
once per compiler process, so a `@theme` edit lands on the next build rather
than on the current dev server.
:::
