...
`[class~="shadow-md"]` counts. It does not yet read a Vue or Svelte `<style>`
block, or a class named inside an arbitrary variant in your markup
(`group-[.shadow-md]:p-2`). For a class matched there, pass it last to `szcn`,
or leave out the class that covers it.
