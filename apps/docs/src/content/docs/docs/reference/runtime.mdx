...
`classify('text-sm')` reports `size`, both in category `text`. It is the same
classification `szcn` merges by, so a token name declared in the app's Tailwind
`@theme` (see [Theme group registry](#theme-group-registry)) is read the way a
built-in one is. The field is **absent** when the prefix means exactly one
