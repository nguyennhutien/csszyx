...
Because the table comes from your compiled CSS, custom `@theme` tokens,
`@utility` classes and plugin utilities merge the same way built-in ones do,
...
evidence from compiled CSS. If a merge you relied on stopped happening, the
build has no table for that class — check the lane table below.
</Aside>
