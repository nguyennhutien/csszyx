...
- **`@csszyx/runtime/split`** — the className half of the class toolkit
  ...
  the main entry already tree-shakes to within 76&nbsp;B of it; the saving is on
  the `require()` path, where nothing shakes: 22.5&nbsp;KB gz against
  30.9&nbsp;KB. It does not drop `@csszyx/compiler` from the dependency graph:
