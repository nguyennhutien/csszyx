...
#### Keys that cannot take a runtime value
...
<div sz={{ textAlign: align }} />  // → dropped + reported    ❌ text-(--v) is a COLOUR
<div sz={{ display: how }} />      // → dropped + reported    ❌ display-(--v) is not a utility
```

`color` and `textAlign` share the `text-` prefix and only one of them has a var
form, so the distinction is per **key**, never per prefix. Both failure shapes
used to be silent: the first wrote `color: var(…)` holding something like
`"center"`, an invalid value that unsets the inherited colour; the second put a
class in the safelist that Tailwind generated no rule for.

The keys are the keyword-valued ones — layout and display modes, alignment,
overflow, text alignment and transform, background sizing and repeat, border
and outline *style*, and their neighbours. There is no need to memorise the
list: the build names the key when you hit it.

The fix is to give the compiler the vocabulary:

```tsx
const align = szv({
    variants: { align: { left: { textAlign: 'left' }, center: { textAlign: 'center' } } },
});
<div sz={align({ align: side })} />
```

Static values on these keys were never affected, and neither is the runtime —
`_sz({ textAlign: side })` receives a real value and has always produced the
keyword utility.
