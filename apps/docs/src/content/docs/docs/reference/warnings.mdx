...
| `Registered custom property {name} is not aliasable in Phase H v1.` | `@property` gives the name a type and inheritance behaviour an alias would not carry | Drop it from `tokens` |
