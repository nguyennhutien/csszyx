...
| `[csszyx] production.mangleMapDelivery: '{value}' emits an executable inline <script> into the built HTML, which a strict Content-Security-Policy (script-src 'self') refuses. 'bundle' (the default) registers the map from inside the JS bundle instead and needs no CSP exception; '{value}' is deprecated and will be removed in a future major.` | Warning, once per build, vite/rollup lanes, only when mangling is on |
