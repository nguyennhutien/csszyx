...
option ([configuration](/docs/reference/config/)).
...
`{after}` read `none` when no prefix is set. Nothing to do.
...
A webpack watch **rebuild error**. webpack keeps the modules it already
compiled and a plugin cannot make it compile them all again, so every rebuild
fails with this until the prefix is back or the watch restarts.
...
(`the dev server`) or a webpack watch (`this watch session`). It is a warning
...
### The Tailwind prefix on the Next.js and Jest lanes
