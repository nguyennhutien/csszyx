...
| `[csszyx] No prebuilt native binary (@csszyx/core-*) is available for this platform, so the default ``rust`` parser fell back to ``oxc``. Parse speed differs, and so does one shape: a const a file declares and then uses as an sz value compiles to a static utility under ``rust`` and to a runtime CSS variable under ``oxc``. To use the native engine, install the matching @csszyx/core-<platform> package (or do not omit optional dependencies). Set ``build.parser`` explicitly to silence this.` |
