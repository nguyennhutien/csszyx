...
| `[csszyx] {filename}: the merge table is in format {n}, and this engine reads format {m}, so no sz key was merged with a later one it covers.` + `help: install the same csszyx version of every @csszyx package, then rebuild.` | correctness | The plugin and the engine come from different csszyx releases. Every class of the file is kept, as before 0.18, so `{ pb: 2, p: 4 }` emits both. Align the `@csszyx/*` versions. |
