...
written in plain CSS joined a merge group. The migration requires both an
`@utility` declaration and an integration that supplies the merge table.
Without that table, `szcn` removes only exact repeats.
...
key covers — a `@theme` token added or removed, an `@utility` rewritten. Each
