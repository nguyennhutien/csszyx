...
half of that gate and do reach the console. The rows marked **production
builds too** say a class is dead, and the bundler plugin prints them in a
production build as well — only `quiet: true` silences them.
</Aside>
...
| `[csszyx] "{key}: {value}"{ at LOC} is not a {key} value. The class "{class}" is still emitted and styles nothing, unless a rule of your own happens to match it. {key} takes one of: {values}.` | `display`, `position`, `visibility` and `isolation` only. **Production builds too.** See below. |
| `[csszyx] "{key}"{ at LOC} is not a variant, but it holds an object, so it lowers to the class prefix "{key}:" and Tailwind generates no CSS for it. A "--*" key takes a declaration value; "container" takes true.` | The class *is* emitted. Only `--*` and `container` are reported — see below. **Production builds too.** |
