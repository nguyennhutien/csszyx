...
leaves its directory out for the loader too.
...
An **error** from `csszyx next prebuild` and `csszyx next watch`. Each `--ignore`
pattern is matched on its own, so `!legacy/keep.css` would not take one file
