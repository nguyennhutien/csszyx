...
| `[csszyx] "{key}: {value}"{ at LOC} is not a {key} value — nothing is emitted for it. {key} takes one of: {values}.` | `display`, `position`, `visibility` and `isolation` only. See below. |
| `[csszyx] "{key}"{ at LOC} is not a variant, but it holds an object, so it lowers to the class prefix "{key}:" and Tailwind generates no CSS for it. A "--*" key takes a declaration value; "container" takes true or a name.` | The class *is* emitted. Only `--*` and `container` are reported — see below. |
...
#### Four keys spell their value as the class, so a typo is refused
...
That made an unrecognised value worse than a dead class: it shipped verbatim as
a single unprefixed class name, which is the shape a project's own component
CSS is made of, so `{ display: 'bogus' }` could match a `.bogus` rule that was
never meant for it.

CSS closes all four value sets, so csszyx refuses a value outside them:

```tsx
<div sz={{ display: 'flex' }} />   // ✅ flex
<div sz={{ display: 'none' }} />   // ✅ hidden
<div sz={{ display: 'bogus' }} />  // ❌ nothing emitted, warned with the legal set
```
