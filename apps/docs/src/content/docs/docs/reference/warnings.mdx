...
almost all wording is byte-identical. Two known wording differences:

| Case | Babel / oxc | Native engine |
| --- | --- | --- |
| `szRecover` with an unsupported value | Two distinct messages (non-literal vs unknown mode) | One combined message: `only static string-literal values "csr" or "dev-only" are supported. Token emission skipped.` |
