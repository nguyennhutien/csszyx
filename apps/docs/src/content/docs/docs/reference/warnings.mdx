...
| `Classes the toolkit reads as utilities, which this project does not serve:` + one line per offender reading `"{token}" is read as a {category} utility and routed to the {node} node, but this project's Tailwind produces no CSS for it. help: if it is your own class, pin it with { {role}: ['{token}'] }.` | A className that collides with a utility prefix — `tab-items-wrapper` reads as `tab-size`. See below. |
| `These are warnings, not failures: a custom @utility or a plugin class can look the same.` + `If one is your own class, pin it at the splitBox call: { {role}: ['{token}'] }.` | **Does not exit non-zero**, unlike every other pass here. |
...
### A class the toolkit reads with false confidence

`classify` matches a className by class PREFIX and knows nothing about which
values a prefix accepts, so an app's own `tab-items-wrapper` reads as a
`tab-size` utility and `splitBox` routes it to a node by a rule rather than by
the fallback. The runtime warns about a class it cannot classify; it cannot
warn about this one, because from inside the runtime the answer looks certain.
Measured against Tailwind 4.3, every prefix in the generated table accepts at
least one suffix Tailwind does not serve.

The pass fires only when both hold: `classify` answered confidently, and every
stylesheet in the project produces no CSS for the token. A class nothing
classifies stays quiet here — that one is the runtime's warning.

It **warns without failing the run**, which is deliberate and unlike the other
passes in this command. The boundary between a utility and an app's own class
is genuinely blurry: a custom `@utility`, a class from a third-party plugin, a
name built by concatenation. A check that fails a commit on a blurry call is
one a team switches off, and a check nobody runs catches nothing. `--json`
carries every finding under the `misclassified-class` rule, so a project that
wants to gate on it can read the report and decide for itself.

Because the oracle is the project's own design system, a class the project
declares itself — `@utility tab-items-wrapper { … }` — is a real class here and
is not reported.
