...
is the cause. On Next.js the transformer follows the `--ignore` patterns that
`csszyx next prebuild` or `csszyx next watch` recorded; a suite that runs before
either, as on a fresh CI checkout, has no record to follow and needs
`tailwindStylesheet`.
