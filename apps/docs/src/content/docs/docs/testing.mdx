...
1. **The build's transform cache**, `.csszyx/cache/transform`. It holds the
   ...
   [object rule](/docs/sz-props/#a-later-key-covers-an-earlier-one), and a
   per-file compile has no stylesheet either, so where a build merged a later
   `sz` key over an earlier one it covers (`{ pb: 2, p: 4 }` → `p-4`) a test
   still sees both classes. Assert `toHaveClass` on the class you rely on, not
   on the whole string, and read a class list from a suite as the unmerged one.
