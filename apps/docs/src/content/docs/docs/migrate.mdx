...
The same CLI ships `csszyx check`, a static diagnostic pass that scans every
...
```bash
npx @csszyx/cli check                     # scan the whole project
npx @csszyx/cli check --pattern "src/**/*.tsx"
```
...
This is a transitional helper for the single-way-keys upgrade and will be removed
at v1.
:::
...
These two runtime helpers are gone from `@csszyx/runtime`. Compose `_sz` with a
...
_sz("base", _szIf(isActive, "active", "inactive"));
_sz("base", _szSwitch(size, { sm: "p-2", lg: "p-6" }));

// after
import { _sz } from "@csszyx/runtime";
_sz("base", isActive ? "active" : "inactive");
_sz("base", { sm: "p-2", lg: "p-6" }[size]);
```
...
On a file an earlier pass already migrated, an element reads
`className="mystery" sz={{ … }}`. The resolve pass merges the classes the map
decides into that existing sz object and drops them from `className`; a key the
sz object already sets is never overridden — the element is left as is and
reported, so you resolve it by hand. Classes the map does not decide stay in
`className` and keep their `@sz-todo` marker.
...
`migrate` runs on the native engine and needs the `@csszyx/core-<platform>`
package for your platform. npm installs it for you on Linux, macOS and Windows,
for x64 and arm64 — it is an optional dependency resolved at install time.

There is no second implementation behind it. On a platform with no prebuilt
binary, or after an install that skipped optional packages, migrate reports the
missing package and stops rather than producing different output:

```
csszyx migrate needs the native engine, and this install has none.
Install the optional package for this platform: @csszyx/core-linux-x64-gnu.
```

Building and the runtime are unaffected by this — those fall back to the WASM
build of the engine that ships inside `@csszyx/core`. migrate is the one command
that cannot, because the WASM artifact is built without it.

:::note[Removed in 0.15.0]
`CSSZYX_MIGRATE_ENGINE` selected between a Rust and a TypeScript engine while
both existed. The TypeScript one is gone, so the variable has nothing to choose
and is ignored. Remove it from any script that sets it.
:::
