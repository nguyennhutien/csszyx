...
- `build.mergeCoveredKeys` (default TRUE) merges a later key of a static `sz` object over an earlier key whose CSS it fully covers, read from the project's compiled Tailwind (`{ pb: 2, p: 4 }` → `p-4`); set FALSE only when that reading is wrong for a project and a class you need is removed — the build then keeps every class as before 0.18; on Turbopack pass `csszyxTurbopack({ mergeCoveredKeys: false })`
