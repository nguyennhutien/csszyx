...
- `build.importedStaticSz` (default false) compiles a static sz object imported from another module — v1 covers a direct `sz={binding}` from a relative NAMED import of an `export const` object literal; barrels, package specifiers, tsconfig aliases, namespace/default imports, nested positions and the Next.js Turbopack lane keep the runtime fallback and report it
