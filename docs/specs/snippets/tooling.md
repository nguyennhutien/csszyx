...
affected) — not a plugin fault. Two known causes, in likelihood order: an IME in
composition mode (Vietnamese Telex/VNI, CJK — its underline appears beneath
typed letters and an open list stops filtering; switching to plain English input
fixes it) and AI inline suggestions (microsoft/vscode#315373). Verify by typing
`Ma` on an empty line.
