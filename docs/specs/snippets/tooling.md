...
but typing letters never opens suggestions while `.`/`'` do, the editor's
suggest widget is suppressed by AI inline suggestions (microsoft/vscode#315373)
— an editor-wide condition (TypeScript's own completions are equally affected),
not a plugin fault. Verify by typing `Ma` on an empty line.
