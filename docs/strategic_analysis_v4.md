...
**Counter:** This task is more about *creating tooling* than *running tests*. It requires:
...
**Counter:** Executor role typically handles *running* established scripts, not *designing* complex parsers from scratch.
...
**Critical Question:** *Is the transform function actually perf-critical?*
...
3. **WASM Escape Hatch:** Design the generator interface so it *can* be swapped for WASM later, but don't block on it.
