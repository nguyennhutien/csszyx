...
function render({ prefixes, tokens, keyRoles }) {
    ...
    // Carries the entry, not just the name: a Set would force the runtime to
    // look the token up a second time and branch on a miss this generator has
    // already made impossible.
    const markerEntries = [...SCOPE_MARKERS.tokens]
        .sort()
        .map(t => entry([t, tokens.get(t)]))
        .join('\n');
