...
function clone(first, second) {
    return {
        ...
        firstFile: { name: `/repo/${first.path}`, start: first.start, end: first.end },
        secondFile: { name: `/repo/${second.path}`, start: second.start, end: second.end },
    };
}

describe('changed-line duplication', () => {
    const both = clone(
        { path: 'packages/a/src/x.ts', start: 10, end: 24 },
        { path: 'packages/a/src/y.ts', start: 80, end: 94 },
    );

    it('keeps a clone whose first block covers a changed line', () => {
        const touched = new Map([['packages/a/src/x.ts', new Set([12])]]);

        assert.equal(clonesOnChangedLines([both], touched, '/repo').length, 1);
    });
    ...
    it('drops a clone the diff did not touch', () => {
        ...
        assert.deepEqual(clonesOnChangedLines([both], touched, '/repo'), []);
    });
