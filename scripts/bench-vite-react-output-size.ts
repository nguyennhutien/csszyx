...
/**
 * Benchmarks playground/vite-react production output size with CSS variable
 * mangling off and on. The playground has focused dynamic CSS variable
 * fixtures, so this is a more representative #11 savings signal than docs.
 *
 * Usage:
 *   pnpm bench:vite-react-size
 */
...
const APP_ROOT = join(REPO_ROOT, 'playground/vite-react');
const DIST_ROOT = join(APP_ROOT, 'dist');
const REPORT_MD = join(REPO_ROOT, '.agent/reports/phase-f-vite-react-mangle-vars-size-bench.md');
const REPORT_JSON = join(
    REPO_ROOT,
    '.agent/reports/phase-f-vite-react-mangle-vars-size-bench.json',
);
const BUILD_OUTPUTS = [DIST_ROOT, join(APP_ROOT, '.csszyx/cache/transform')];
...
function runBuildCase(mode: BenchMode): BenchRow {
    ...
    const result = spawnSync('pnpm', ['--filter', '@csszyx/playground-vite-react', 'build'], {
        cwd: REPO_ROOT,
        encoding: 'utf8',
        env: {
            ...process.env,
    ...
    if (result.status !== 0) {
        return {
            name: `vite-react-output-size/${mode}`,
            mode,
            status: 'failed',
            groups: emptyGroups(),
    ...
    return {
        ...
        note:
            mode === 'mangle-vars-on'
                ...
                : 'Rust parser build with CSSZYX_BENCH_MANGLE_VARS=0.',
    };
}
...
function summarizeFailure(output: string): string {
    const lines = output
        ...
        .filter(Boolean);
    return (
        lines.find(line => /error|csszyx|vite|tailwind/i.test(line)) ??
        lines.at(-1) ??
        'build failed'
    )
        .slice(0, 240)
...
function renderReport(payload: ReportPayload): string {
    ...
    return `# Phase F Vite React Mangle Vars Size Benchmark
...
- This is a production artifact-size bench for the focused Vite React
  playground, which includes the browser-facing CSS variable fixture.
- The playground keeps \`production.mangleVars\` enabled by default. The bench
  uses \`CSSZYX_BENCH_MANGLE_VARS=0\` only to build a disabled comparison row.
- Raw bytes show emitted artifact size. Gzip and brotli bytes better approximate
  transfer size and should guide any future default-flip decision.
`;
}
