//! TS↔Rust parity for `csszyx migrate` on whole files.
//!
//! The corpus is generated by `scripts/gen-migrate-source-parity-corpus.mjs`:
//! hand-written snippets that reach every branch of the transformer,
//! synthetic files in the benchmark's shapes, and the real components under
//! apps/docs, each recorded under the option sets that matter for it. The
//! Rust transformer must write the same code, report the same stats and
//! warnings, and name the same unused imports. A parse failure is the one
//! place the two parsers may word things differently, so only the warning's
//! prefix is compared there.
