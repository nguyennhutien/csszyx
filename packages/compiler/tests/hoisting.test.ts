...
describe('hoistCSSVariables', () => {
    it('should not hoist when only one usage', () => {
        ...
        const style = element.attributes.find(
    ...
    it('should skip hoisting when serializedValue is null (dynamic)', () => {
        ...
        for (const el of [el1, el2]) {
            const style = el.attributes.find(
                a => t.isJSXAttribute(a) && t.isJSXIdentifier(a.name) && a.name.name === 'style',
            );
            expect(style).toBeDefined();
        }
        ...
        const usages: CSSVarUsage[] = [
            {
                element: el1,
                varName: '--_sz-p',
                valueExpr: t.stringLiteral('val1'),
                serializedValue: 'S:val1',
            },
            {
                element: el2,
                varName: '--_sz-p',
                valueExpr: t.stringLiteral('val2'),
                serializedValue: 'S:val2',
            },
        ];
        const parentMap = new Map<t.Node, t.Node>();
    ...
    it('should NOT hoist when same var has different values', () => {
        ...
        const usages: CSSVarUsage[] = [
            ...
            },
            {
                element: el2,
                varName: '--_sz-p',
                valueExpr: t.stringLiteral('calc(8 * var(--spacing))'),
                serializedValue: 'S:calc(8 * var(--spacing))',
            },
        ];
        const parentMap = new Map<t.Node, t.Node>();
        ...
        const usages: CSSVarUsage[] = [
            {
                element: el,
                varName: '--_sz-m',
                valueExpr: t.stringLiteral('val'),
                serializedValue: 'S:val',
            },
        ];
        const parentMap = new Map<t.Node, t.Node>();
        ...
        const usages: CSSVarUsage[] = [
            {
                element: el1,
                varName: '--_sz-p',
                valueExpr: t.stringLiteral('static-val'),
                serializedValue: 'S:static-val',
            },
            {
                element: el2,
                varName: '--_sz-p',
                valueExpr: t.identifier('dynamicVar'),
                serializedValue: null,
            },
        ];
        const parentMap = new Map<t.Node, t.Node>();
...
describe('buildParentMap edge cases', () => {
    it('should handle deeply nested AST', () => {
        ...
        const stmt = t.expressionStatement(t.callExpression(t.identifier('fn'), [deepLiteral]));
