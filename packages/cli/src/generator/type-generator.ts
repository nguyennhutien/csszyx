...
/**
 * Generate a union whose values are safe single-quoted TypeScript literals.
 *
 * @param values Raw theme values to join.
 * @returns Union type string such as `'val1' | 'val2'`.
 */
function generateUnionType(values: string[]): string {
    if (values.length === 0) {
        return 'string';
    }
    return values.map(v => `'${escapeSingleQuotedString(v)}'`).join(' | ');
}
