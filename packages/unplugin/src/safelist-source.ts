...
/** Text every safelist csszyx has ever written carries in its header. */
const CSSZYX_MARKERS = ['Auto-generated by csszyx', 'csszyx Next safelist'];
...
export function removeLegacySafelists(rootDir: string, warn: (message: string) => void): void {
    for (const legacy of LEGACY_SAFELIST_FILES) {
        ...
        if (!CSSZYX_MARKERS.some(marker => content.includes(marker))) continue;
