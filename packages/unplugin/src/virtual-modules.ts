...
import { MERGE_TABLE_FORMAT } from './merge-signature.js';
...
/** Virtual module that registers the names this project's Tailwind does not serve. */
export const UNSERVED_VIRTUAL_ID = 'virtual:csszyx/unserved';

/** Resolved form of {@link UNSERVED_VIRTUAL_ID}. */
export const RESOLVED_UNSERVED_VIRTUAL_ID: string = `\0${UNSERVED_VIRTUAL_ID}`;

/** Vite-only registration update delivered before the source module's hot update. */
export const MERGE_TABLE_UPDATE_EVENT = 'csszyx:merge-table';
...
/**
 * Stands in for the class names the project's Tailwind serves nothing for.
 *
 * A build knows the names only once every module has been transformed, but the
 * import that pulls this module in is written DURING that transform. So the
 * module is emitted with a hole and the hole is filled at bundle time, exactly
 * as the mangle map is.
 */
export const UNSERVED_PLACEHOLDER = '___CSSZYX_UNSERVED___';
export const MERGE_SIGNATURES_PLACEHOLDER = '___CSSZYX_MERGE_SIGNATURES___';

/**
 * Emit an expression that preserves JSON keys such as `__proto__` as data.
 * O(b) work for b serialized bytes; the generated module pays one JSON parse.
 * @param table - Finalized app-scoped signature data.
 * @returns A script-safe JavaScript expression for the table.
 */
export function serializeMergeSignatures(table: MergeSignatureTable): string {
    return `JSON.parse(${escapeJsonForInlineScript(JSON.stringify(JSON.stringify(table)))})`;
}

/**
 * The module that hands the runtime the names Tailwind serves nothing for.
 *
 * `splitBox` then places them by the fallback, the way it already places a
 * class it cannot classify at all -- the two are the app's own vocabulary and
 * differ only by an accident of prefix matching.
 *
 * @param classes - The names, when the caller already knows them. A dev server
 * does, because its prescan walked every source before the first module loaded,
 * and it needs them here: nothing renders a chunk, so a hole left for bundle
 * time would reach the browser as an undefined identifier.
 * @param mergeSignatures - App-scoped merge table, when already finalized.
 * @param format - `cjs` for the twin a jest suite requires; jest in its
 * default mode cannot load an ES module. `vite` adds a dev-only hot listener;
 * production bundlers take `esm`. Emission is O(b) in serialized bytes, including
 * arbitrary class names. Hot delivery replaces state in O(u) unserved names;
 * registering the merge table itself is O(1), with no module fetch or evaluation.
 * @returns Source for the generated registration module.
 */
export function createUnservedRuntimeModule(
    classes?: readonly string[],
    mergeSignatures?: MergeSignatureTable,
    format: 'esm' | 'cjs' | 'vite' = 'esm',
): string {
    const names = '{ registerMergeSignatures, registerUnservedClasses }';
    const binding =
        format !== 'cjs'
            ? `import ${names} from '@csszyx/runtime';`
            : `const ${names} = require('@csszyx/runtime');`;
    return `/**
...
registerUnservedClasses(${classes === undefined ? UNSERVED_PLACEHOLDER : JSON.stringify(classes)});
registerMergeSignatures(${mergeSignatures === undefined ? MERGE_SIGNATURES_PLACEHOLDER : serializeMergeSignatures(mergeSignatures)}, { format: ${MERGE_TABLE_FORMAT} });
${
    format === 'vite'
        ? `
if (import.meta.hot) {
    import.meta.hot.on('${MERGE_TABLE_UPDATE_EVENT}', ({ classes, table, format }) => {
        registerUnservedClasses(classes);
        registerMergeSignatures(table, { format });
    });
}
`
        : ''
