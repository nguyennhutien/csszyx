...
describe('the Vite writer sweeps the pre-0.15.0 safelist', () => {
    it('removes the old HTML file the plugin itself wrote and says where the safelist went', async () => {
        const { root, hotUpdate } = await bootedPlugin();
        const legacy = path.join(root, 'csszyx-classes.html');
        fs.writeFileSync(
            legacy,
        ...
        expect(fs.existsSync(path.join(root, '.csszyx/csszyx-classes.txt'))).toBe(true);
        expect(fs.existsSync(legacy)).toBe(false);
        expect(warn.mock.calls.map(call => String(call[0])).join('\n')).toContain(
            'removed csszyx-classes.html',
        );
        warn.mockRestore();
    });
});
